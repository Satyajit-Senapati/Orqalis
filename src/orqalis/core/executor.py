...
class RunExecutor:
        ...
        *,
        ...
        if run.state == RunState.GOAL_DEFINED:
            self.orchestrator.install_plan(
                VerticalPlanner().plan(contract, context), "vertical:plan"
            )
        ...
        await self._context_wave(run_id, provider_id, context, policy.max_parallel_tasks)
        for role in (AgentRole.REPAIR, AgentRole.DEVELOPER, AgentRole.TESTER, AgentRole.REVIEWER):
            ...
            for task in tasks:
                ...
                if role == AgentRole.DEVELOPER:
                    await self._context_wave(
                        run_id, provider_id, context, policy.max_parallel_tasks
                    )
        with self.factory() as uow:
            current = locked_run(uow, run_id)
    ...
    async def _context_wave(
        self, run_id: UUID, provider_id: str, context: ContextPack, capacity: int
    ) -> None:
    ...
    async def _context_task(
        self, run_id: UUID, task: Task, provider_id: str, context: ContextPack
    ) -> None:
        attempt = self._attempt(run_id, task)
        try:
            output = await self.worker.execute(
