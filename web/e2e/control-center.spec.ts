...
test.skip(!runId, "Set ORQALIS_E2E_COMPLETED_RUN_ID to the completed execution fixture");

test("Operational views reconstruct a delivered run with keyboard and theme support", async ({ page, request }, info) => {
  const errors: string[] = [];
  page.on("pageerror", error => errors.push(error.message));
  await page.goto(`/runs/${runId}`);
  await expect(page.getByRole("heading", { name: "Normalize names consistently" })).toBeVisible();
  ...
  const intervals = await (await request.get(`/api/runs/${runId}/timeline`)).json() as unknown[];
  await expect(page.locator(".timeline-bar")).toHaveCount(intervals.length);
  await page.locator(".timeline-bar").first().click();
  await page.screenshot({ path: info.outputPath("timeline-dark.jpg"), type: "jpeg", quality: 65 });
  await page.getByRole("tab", { name: "Acceptance", exact: true }).click();
  await page.locator(".evidence-criterion").first().locator("summary").first().click();
  await expect(page.locator(".evidence-criterion").first().locator(".evidence-record")).toHaveCount(2);
  ...
  await expect(page.getByRole("region", { name: "Project knowledge graph" })).toBeVisible();
  await page.getByRole("tab", { name: "Delivery", exact: true }).click();
  await expect(page.locator(".delivery-checkpoints .status-pass")).toHaveCount(6);
  await page.getByRole("button", { name: "main.py", exact: true }).click();
  await expect(page.locator(".diff-view")).toContainText("+    return value.strip().lower()");
  await page.getByLabel("Color theme").selectOption("light");
  await page.screenshot({ path: info.outputPath("delivery-light.jpg"), type: "jpeg", quality: 65 });
  await page.getByRole("tab", { name: "Metrics", exact: true }).click();
  await expect(page.getByRole("heading", { name: "Runtime statistics" })).toBeVisible();
  ...
  await expect(page.getByRole("table", { name: "Persisted run comparison" })).toBeVisible();
  await page.setViewportSize({ width: 390, height: 844 });
  await page.screenshot({ path: info.outputPath("metrics-mobile.jpg"), type: "jpeg", quality: 65 });
